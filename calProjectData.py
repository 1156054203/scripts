#!/usr/bin/env python3
# created by ChenYulong@20181026
# the statistics of rawdata,cleandata,cleanrate,meancov,maprate,cov(Y/X),seqtype,testitem,datapath for project
import os
import sys
import glob
import argparse
...
def parse_args():
    usage = "python calProjectData.py -p project -o outfile -t [wgs,wes,panel]"
    ...
    if os.path.isdir(filepath):
        if os.listdir(filepath):
           ...
           sample.extend(list(os.walk(filepath))[0][1])
            ...
            sys.exit()
    f_out=open(output,'w')
    f_out.write('Project'+'\t'+'Sample'+'\t'+'RawData(Gb)'+'\t'+'CleanData(Gb)'+'\t'+'Ratio(%)'+'\t'+'MapBase'+'\t'+'MeanCov'+'\t'+'MapRate(%)'+'\t'+'cov(Y/X)'+'\t'+'Tpye'+'\t'+'Items'+'\t'+'Path'+'\n')
    for sam in sample:
        ...
        if glob.glob(r'./*.json'):
            ...
            ratio=round(int(tmp2)/int(tmp1)*100,2) 
            f_out.write(pro+'\t'+sam+'\t'+str(raw)+'\t'+str(clean)+'\t'+str(ratio)+'\t'+str(num)+'\t'+cover+'\t'+map+'\t'+str(YX)+'\t'+type+'\t'+'/'+'\t'+dir2+'\n')
        else:
...
def cal_wesmapDepth(pro,output):
    ...
    for sam in sample:
        ...
        if glob.glob(r'./*.json'):
            ...
            clean=round(int(tmp2)/10**9,2)
            ratio=round(int(tmp2)/int(tmp1)*100,2)
            f_out.write(pro+'\t'+sam+'\t'+str(raw)+'\t'+str(clean)+'\t'+str(ratio)+'\t'+str(num)+'\t'+cover+'\t'+map+'\t'+'/'+'\t'+type+'\t'+'/'+'\t'+dir2+'\n')
        else:
            print('The fastq files is not in %s' %dir2)
    f_out.close()
    ...
    if os.path.isdir(filepath):
        if os.listdir(filepath):
           ...
           sample.extend(list(os.walk(filepath))[0][1])
            ...
            sys.exit()
    f_out=open(output,'w')
    f_out.write('Project'+'\t'+'Sample'+'\t'+'RawData(Mb)'+'\t'+'CleanData(Mb)'+'\t'+'Ratio(%)'+'\t'+'MapBase'+'\t'+'MeanCov'+'\t'+'MapRate(%)'+'\t'+'cov(Y/X)'+'\t'+'Type'+'\t'+'Path'+'\t'+'Items'+'\n')
    for sam in sample:
