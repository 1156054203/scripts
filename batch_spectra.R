...
tmp1 <- tmp1[order(tmp1$ref,tmp1$alt),]
...
#mono <- mono[order(mono$ref,mono$alt,mono$before),]

library(ggplot2)

stag <- max(prop.table(mono$num))*100+0.1
tag <- max(prop.table(mono$num))*100+0.3
palette <- c('#A52A2A','#FFC125','#B03060','#76EE00','#A0522D','#BCEE68',
               '#9400D3','#8B475D','#87CEFA','#836FFF','#71C671','#6E8B3D')
ggplot(mono,aes(1:96,prop.table(num)*100,fill=type))+
    ...
    annotate('segment',x=0,y=stag,xend=16,yend=stag,color='#A52A2A')+
    ...
    annotate('segment',x=33,y=stag,xend=48,yend=stag,color='#B03060')+
