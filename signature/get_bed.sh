...
for var in `find $prefix/group* -name "*.seq"`;do
    ...
    group=`echo $var|awk -F'/' '{print$7}'`
    out=$prefix/${group}
    cat $out/${name}.isec.anno.vcf | grep -v '^#' |awk -F '[\t|]' '{print$1,$2,$4,$5,$11,$27}'|paste -d '\t' - $out/${name}.5flank.seq $out/${name}.3flank.seq | awk '{OFS="\t"}{print$1,$2,$3,$4,$8,$10,$5,$6}' >$out/${name}.ready.txt
