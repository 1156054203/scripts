...
mode <- read.table('Rmode.txt',header=T,stringsAsFactors=F)
...
ggplot(frame2,aes(context,prop.table(num),fill=type))+
   ...
   theme(legend.position='none',axis.text.x=element_text(vjust=0.5,size=5)
        ...
        ,axis.line.y=element_line(color='black'),strip.text.y=element_text(size=5),plot.margin=unit(c(0,0,0,0.40), "cm"))
tmpname <- strsplit(arg[1],split='\\.')[[1]][1]
ggsave(paste(tmpname,'.png',sep=''),height=674,width=359,units='mm',limitsize=F)
}
